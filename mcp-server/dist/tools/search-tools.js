...
// Tool handlers
export const handleSemanticSearch = async (args) => {
    try {
        ...
        const formattedResults = relevantResults.map((result, index) => `**Result ${index + 1} (Score: ${result.score.toFixed(3)})**\n${result.content}\n---`).join('\n\n');
        return {
            content: [{
                    type: 'text',
...
export const handleSearchExperiences = async (args) => {
    try {
...
export const handleSearchSkills = async (args) => {
    try {
      ...
      `;
        ...
        const formattedSkills = Object.entries(skillsByCategory).map(([cat, catSkills]) => {
            const skillList = catSkills.map((skill) => {
...
export const handleSearchProjects = async (args) => {
    try {
