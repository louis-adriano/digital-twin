...
// Tool handlers
export const handleGetCompleteProfile = async (args: any): Promise<CallToolResult> => {
  try {
    ...
    if (sections.includes('profile')) {
      ...
      if (profile) {
        ...
        if (profile.title) completeProfile += `*${profile.title}*\n`;
        if (profile.location) completeProfile += `📍 ${profile.location}\n`;
        ...
        if (profile.linkedin_url || profile.github_url || profile.website_url) {
          ...
          if (profile.linkedin_url) completeProfile += `• [LinkedIn](${profile.linkedin_url})\n`;
          if (profile.github_url) completeProfile += `• [GitHub](${profile.github_url})\n`;
          if (profile.website_url) completeProfile += `• [Website](${profile.website_url})\n`;
          completeProfile += `\n`;
        }
      }
    }
    ...
    if (sections.includes('experiences')) {
      ...
      if (experiences.length > 0) {
        ...
        experiences.forEach((exp: any, index: number) => {
          ...
          if (exp.location) completeProfile += ` | 📍 ${exp.location}`;
          completeProfile += `\n\n`;
    ...
    if (sections.includes('education')) {
      ...
      if (education.length > 0) {
        ...
        education.forEach((edu: any) => {
          ...
          if (edu.field_of_study) completeProfile += ` in ${edu.field_of_study}`;
          completeProfile += `\n`;
...
export const handleGetCareerProgression = async (args: any): Promise<CallToolResult> => {
  try {
    const { focus } = args;
    ...
    if (focus === 'roles') {
      ...
      experiences.forEach((exp: any) => {
        if (exp.technologies) {
          exp.technologies.forEach((tech: string) => uniqueTech.add(tech));
        }
      });
...
export const handleGenerateProfessionalSummary = async (args: any): Promise<CallToolResult> => {
  try {
    ...
    summary += introductions[audience as keyof typeof introductions] || introductions.general;
    summary += `\n\n`;
    ...
    if (experiences.length > 0 && length !== 'brief') {
      ...
      experiences.slice(0, expCount).forEach((exp: any) => {
    ...
    if (focus_area) {
      ...
      const relevantSkills = skills.filter((skill: any) => 
        skill.name.toLowerCase().includes(focus_area.toLowerCase()) ||
        skill.category.toLowerCase().includes(focus_area.toLowerCase())
      );

      const relevantExperiences = experiences.filter((exp: any) => 
        (exp.description && exp.description.toLowerCase().includes(focus_area.toLowerCase())) ||
        (exp.technologies && exp.technologies.some((tech: string) => 
          tech.toLowerCase().includes(focus_area.toLowerCase())
        ))
      );

      if (relevantSkills.length > 0) {
        summary += `Skills: ${relevantSkills.map((s: any) => s.name).join(', ')}\n`;
      }

      if (relevantExperiences.length > 0) {
        summary += `Applied in roles: ${relevantExperiences.map((e: any) => `${e.position} at ${e.company}`).join(', ')}\n`;
      }
      summary += `\n`;
    }
    ...
    summary += closings[audience as keyof typeof closings] || closings.general;
